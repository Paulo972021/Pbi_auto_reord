...
FILTER_PLAN: dict = {
    "entrante": {
        ...
        "mode": "single",
        "clear_first": True,
        "target_values": ["VEICULOS"],
        "required": True,
    },
}

OUTPUT_BASE_DIR = "output"
DOWNLOAD_ALLOWED_EXTENSIONS = {".xlsx", ".csv", ".txt", ".tsv", ".zip"}
DOWNLOAD_TEMP_EXTENSIONS = {".crdownload", ".tmp", ".part"}
...
async def get_visual_header_state(tab, visual) -> dict:
    """Diagnóstico do header/botão de opções para probe de exportação."""
    idx = visual.get("index")
    title = visual.get("title", f"Visual #{idx}")
    page_visibility = await get_page_focus_state(tab, stage_label=f"export_probe_visual_{idx}")
    log.info(
        f"[EXPORT_PAGE_VISIBILITY] visual_index={idx} "
        f"hidden={page_visibility.get('hidden')} "
        f"visibilityState={page_visibility.get('visibilityState')} "
        f"hasFocus={page_visibility.get('hasFocus')}"
    )
            ...
            (() => {{
                ...
                if (!vc) {{
                    return JSON.stringify({{
                        ...
                        buttons_found: 0,
                        target_button_visible: false,
                        target_button_enabled: false,
                ...
                const visual_rendered = vcRect.width > 10 && vcRect.height > 10;
                const header = vc.querySelector('visual-container-header, .visualContainerHeader, [class*="header"]');
                const header_visible = isVisible(header);
                let buttons = Array.from(vc.querySelectorAll('button')).filter((b) => {{
                    const r = b.getBoundingClientRect();
                    return r.width > 1 && r.height > 1 && r.top >= vcRect.top && r.top <= vcRect.top + 56;
                }});
                if (!buttons.length) {{
                    buttons = Array.from(vc.querySelectorAll('button')).filter(isVisible);
                }}
                buttons.sort((a, b) => b.getBoundingClientRect().left - a.getBoundingClientRect().left);
                const target = buttons.length ? buttons[0] : null;
                return JSON.stringify({{
                    ok: true,
                    header_visible,
                    buttons_found: buttons.length,
                    target_button_visible: isVisible(target),
                    target_button_enabled: target ? !target.disabled : false,
                    visual_rendered,
                }});
            }})()
        """)
    ...
    result = {
        ...
        "header_visible": bool(parsed.get("header_visible")),
        "buttons_found": int(parsed.get("buttons_found") or 0),
        "target_button_visible": bool(parsed.get("target_button_visible")),
        "target_button_enabled": bool(parsed.get("target_button_enabled")),
        "visual_rendered": bool(parsed.get("visual_rendered")),
    }
    background_suspected = bool(
        page_visibility.get("hidden") is True
        or str(page_visibility.get("visibilityState") or "").lower() != "visible"
        or page_visibility.get("hasFocus") is False
        or result["buttons_found"] == 0
        or not result["header_visible"]
    )
    result["background_suspected"] = background_suspected
    log.info(
        f"[VISUAL_HEADER_STATE] visual_index={idx} title={title} "
        f"header_visible={result['header_visible']} buttons_found={result['buttons_found']} "
        f"target_button_visible={result['target_button_visible']} "
        f"target_button_enabled={result['target_button_enabled']} "
        f"background_suspected={result['background_suspected']}"
    )
    return result


def classify_export_probe_failure(header_state: dict, click_reason: str, menu_opened: bool, technical_error: str | None) -> str:
    """Classifica falha de probe/exportação com motivo concreto."""
    if technical_error:
        return "js_transport_error"
    if menu_opened:
        return "click_executed_menu_not_opened"
    if (header_state or {}).get("buttons_found", 0) == 0:
        return "no_buttons_rendered"
    if not (header_state or {}).get("header_visible", False):
        return "header_hidden"
    if not (header_state or {}).get("target_button_visible", False):
        return "target_button_not_visible"
    if (header_state or {}).get("background_suspected"):
        return "background_visibility_suspected"
    if click_reason == "menu_not_opened":
        return "click_executed_menu_not_opened"
    return "background_visibility_suspected"
...
async def open_more_options_robust(tab, visual, attempt: int, retries: int) -> dict:
    """
    Abre 'Mais opções' com diagnóstico robusto para background na fase de exportação.
    Retorna dict com status, motivo e se precisou fallback de foco.
    """
    idx = visual.get("index")
    title = visual.get("title", f"Visual #{idx}")
    ...
    async def _attempt_click_once():
        ...
        try:
            eval_click = await guarded_evaluate(
                ...
                (() => {{
                    ...
                    ['pointerenter','pointerover','mouseenter','mouseover','mousemove','pointermove'].forEach(t => {{
                        vc.dispatchEvent(new PointerEvent(t, {{
                            bubbles: true, composed: true, view: window, clientX: hx, clientY: hy
                        }}));
                    }});
                    let btns = Array.from(vc.querySelectorAll('button')).filter(b => {{
                        const r = b.getBoundingClientRect();
                        return r.width > 1 && r.height > 1 && r.top >= vcRect.top && r.top <= vcRect.top + 50;
                    }});
                    if (btns.length === 0) {{
                        btns = Array.from(vc.querySelectorAll('button')).filter(b => {{
                            const r = b.getBoundingClientRect();
            ...
            log.warning(
                f"[EXPORT_TECHNICAL_INCIDENT] visual_index={idx} title={title} "
                f"error={technical_error} retry_allowed={attempt < retries}"
            )
            result_raw = None
        ...
        if result_raw is None:
            ...
            try:
                ...
                result = {"ok": False, "reason": f"parse_err:{str(result_raw)[:60]}"}
        elif isinstance(result_raw, dict):
            result = result_raw
        else:
            result = {"ok": False, "reason": f"type:{type(result_raw).__name__}"}
        ...
        if result.get("ok"):
            for check in range(6):
                ...
                if bool(menu_watch.get("ok") and menu_watch.get("result")):
                    ...
                    break
        failure_reason = classify_export_probe_failure(
            header_state=header_state,
            click_reason=result.get("reason", ""),
            menu_opened=menu_opened,
            technical_error=technical_error,
        )
        return {
            "ok": bool(result.get("ok") and menu_opened),
            "clicked": bool(result.get("ok")),
            "menu_opened": menu_opened,
            "reason": "ok" if result.get("ok") and menu_opened else failure_reason,
            "js_reason": result.get("reason"),
            "header_state": header_state,
        }

    initial = await _attempt_click_once()
    if initial.get("ok"):
        log.info(
            f"[EXPORT_FOCUS_POLICY] visual_index={idx} phase={phase} "
            f"mode=dom_first_then_focus_fallback safe_focus_called=False reason=ok"
        )
        return {"ok": True, "reason": "ok", "focus_fallback_used": False}
    ...
    if needs_focus_fallback:
        ...
        await force_hover_visual_header(tab, visual)
        second = await _attempt_click_once()
        reason = str(second.get("reason") or reason)
        if second.get("ok"):
            log.info(
                f"[EXPORT_FOCUS_POLICY] visual_index={idx} phase={phase} "
    ...
    log.info(
        f"[EXPORT_FOCUS_POLICY] visual_index={idx} phase={phase} "
        f"mode=dom_first_then_focus_fallback safe_focus_called={focus_fallback_used} reason={reason}"
    )
    return {"ok": False, "reason": reason, "focus_fallback_used": focus_fallback_used}
    ...
    for attempt in range(1, retries + 1):
        ...
        if has_export:
            return {
                ...
                "focus_fallback_used": bool(open_result.get("focus_fallback_used")),
            }
        return {
            "menu_opened": True,
            "has_export": False,
...
async def scan_visuals(tab, slicer_indexes: set = None):
    ...
    confirmed_exportable = []
    confirmed_non_exportable = []
    technical_failures = []
    focus_fallback_needed = []
    ...
    for visual in exportable:
        ...
        visual["focusFallbackUsed"] = bool(probe.get("focus_fallback_used"))

        if visual["focusFallbackUsed"]:
            focus_fallback_needed.append(container_idx)
        if visual["hasExportData"]:
            confirmed_exportable.append(container_idx)
        elif visual["exportReason"] in {"js_transport_error"}:
            technical_failures.append(container_idx)
        else:
            confirmed_non_exportable.append(container_idx)

        await close_open_menus_and_overlays(tab, aggressive=True)
        await asyncio.sleep(0.5)

    log.info(
        f"[EXPORT_PROBE_SUMMARY] confirmed_exportable={confirmed_exportable} "
        f"confirmed_non_exportable={confirmed_non_exportable} "
        f"technical_failures={technical_failures} "
        f"focus_fallback_needed={focus_fallback_needed}"
    )
...
def normalize_filter_key(raw_key: str) -> str:
    """Normaliza chave técnica de filtro para comparação estável."""
    return str(raw_key or "").strip().lower()


def load_templates_config():
    """Carrega CATALOG e TEMPLATES do arquivo de configuração único."""
    if config_templates is None:
        log.info("[TEMPLATE_LOAD] templates_count=0 pages=[] source=config_templates.py_not_found")
        return {}, []

    catalog = getattr(config_templates, "CATALOG", {}) or {}
    templates = getattr(config_templates, "TEMPLATES", []) or []

    pages = sorted({str(t.get("page", "")) for t in templates if isinstance(t, dict)})
    log.info(f"[TEMPLATE_LOAD] templates_count={len(templates)} pages={pages}")
    return catalog, templates


def validate_template(template: dict, catalog: dict) -> tuple[bool, list[str], dict]:
    """
    Valida template sem derrubar o lote inteiro.
    Também normaliza as chaves técnicas para runtime.
    """
    errors = []
    normalized = {}
    ...
    if not isinstance(page, str) or not page.strip():
        errors.append("page deve ser string não vazia")

    if not isinstance(filters, dict):
        errors.append("filters deve ser dict")
        filters = {}

    if isinstance(page, str) and page.strip():
        if page not in catalog:
            errors.append(f"page '{page}' não está no CATALOG")

    page_catalog = catalog.get(page, {}) if isinstance(page, str) else {}
    catalog_filters = page_catalog.get("filters", {}) if isinstance(page_catalog, dict) else {}
    catalog_keys_map = {normalize_filter_key(k): k for k in catalog_filters.keys()}

    normalized_filters = {}
    for raw_key, raw_value in filters.items():
        if not isinstance(raw_value, str):
            errors.append(f"filtro '{raw_key}' deve ter valor string")
            continue

        norm_key = normalize_filter_key(raw_key)
        if not norm_key:
            errors.append("chave de filtro vazia")
            continue

        if norm_key not in catalog_keys_map:
            errors.append(f"filtro '{raw_key}' não permitido para page '{page}'")
            continue

        canonical_key = catalog_keys_map[norm_key]
        allowed_values = catalog_filters.get(canonical_key, {}).get("values", [])
        if raw_value not in allowed_values:
            errors.append(
                f"valor '{raw_value}' inválido para filtro '{canonical_key}' "
                f"(permitidos={allowed_values})"
            )
            continue

        normalized_filters[canonical_key] = raw_value

    normalized = {
        "template_id": template_id or "template_sem_id",
        "page": page,
...
def build_runtime_filter_plan(template: dict) -> dict:
    """Converte template validado para FILTER_PLAN runtime (single-select)."""
    runtime = {}
    for filter_key, filter_value in (template.get("filters") or {}).items():
        runtime[normalize_filter_key(filter_key)] = {
            "mode": "single",
            "clear_first": True,
...
def sanitize_template_token(value: str) -> str:
...
def build_template_code(template: dict) -> str:
    """
    Gera código estável do template.
    Ex.: COMPARATIVO__entrante-1__epn_final-ALMAVIVA
    """
    page = sanitize_template_token(template.get("page", "SEM_PAGINA"))
    parts = [page]
    for key in sorted((template.get("filters") or {}).keys()):
        value = template["filters"][key]
        parts.append(f"{sanitize_template_token(key)}-{sanitize_template_token(value)}")
    return "__".join(parts)


def prepare_output_folder(template_id: str, template_code: str) -> str:
    """Recria pasta de output isolada para o template atual."""
    output_folder = os.path.join(OUTPUT_BASE_DIR, template_code)
    previous_removed = os.path.exists(output_folder)
    if previous_removed:
        shutil.rmtree(output_folder, ignore_errors=True)
    os.makedirs(output_folder, exist_ok=True)
    log.info(
        f"[OUTPUT_FOLDER_PREPARE] template_id={template_id} output_folder={output_folder} "
        f"folder_recreated=True previous_contents_removed={previous_removed}"
    )
...
def is_relevant_download_file(filename: str) -> bool:
    lower_name = filename.lower()
    _, ext = os.path.splitext(lower_name)
    if ext in DOWNLOAD_TEMP_EXTENSIONS:
        return False
    return ext in DOWNLOAD_ALLOWED_EXTENSIONS


def cleanup_residual_downloads(downloads_folder: str, template_id: str) -> list[str]:
    """Limpeza controlada de resíduos do processo antes de cada template."""
    removed = []
    if not os.path.isdir(downloads_folder):
        return removed

    for filename in os.listdir(downloads_folder):
        fullpath = os.path.join(downloads_folder, filename)
        if not os.path.isfile(fullpath):
            continue
        if not is_relevant_download_file(filename):
            continue
        try:
            os.remove(fullpath)
    ...
    log.info(
        f"[DOWNLOADS_CLEANUP_BEFORE_TEMPLATE] template_id={template_id} "
        f"files_removed={removed} removed_count={len(removed)}"
    )
    return removed


def snapshot_downloads(downloads_folder: str) -> set[str]:
    if not os.path.isdir(downloads_folder):
        return set()
    return {
...
def capture_and_move_new_downloads(
    template_id: str,
    downloads_folder: str,
    before_files: set[str],
    output_folder: str,
) -> list[str]:
    """Detecta arquivos novos reais e move para a pasta do template."""
    after_files = snapshot_downloads(downloads_folder)
    new_files = sorted(after_files - before_files)
    relevant = [f for f in new_files if is_relevant_download_file(f)]
    ...
    log.info(
        f"[DOWNLOAD_CAPTURE] template_id={template_id} files_detected={len(relevant)} "
        f"files_moved={len(moved)} files_list={moved}"
    )
    return moved
...
async def run_export(
    ...
    filter_plan_override: dict | None = None,
    auto_export_all: bool = False,
):
    ...
    global FILTER_PLAN
    original_filter_plan = FILTER_PLAN
    active_filter_plan = (
        filter_plan_override
        if isinstance(filter_plan_override, dict) and filter_plan_override
        else FILTER_PLAN
    )
    FILTER_PLAN = active_filter_plan
...
#            return True
        ...
        if auto_export_all:
            selected_indexes = exportable
            log.info("📥 Modo batch/template: exportando automaticamente todos os visuais exportáveis.")
        else:
        ...
        FILTER_PLAN = original_filter_plan
...
async def run_export_batch(url: str, browser_path: str, default_target_page: str):
    """
    Executa lote de templates quando houver config, mantendo fallback legado.
    """
    catalog, templates = load_templates_config()
    if not templates:
    ...
    for template in templates:
        template_id = str(template.get("template_id") or "template_sem_id")
        is_valid, errors, normalized_template = validate_template(template, catalog)
        log.info(
            f"[TEMPLATE_VALIDATE] template_id={template_id} valid={is_valid} errors={errors}"
        )
        if not is_valid:
            failed += 1
            continue

        runtime_plan = build_runtime_filter_plan(normalized_template)
        page = normalized_template.get("page") or default_target_page
        template_code = build_template_code(normalized_template)
        log.info(f"[TEMPLATE_CODE] template_id={template_id} template_code={template_code}")
        output_folder = prepare_output_folder(template_id, template_code)
        cleanup_residual_downloads(downloads_folder, template_id)

        log.info(
            f"[TEMPLATE_EXEC_START] template_id={template_id} page={page} "
            f"filters={normalized_template.get('filters', {})}"
        )
        before_files = snapshot_downloads(downloads_folder)
        error_msg = ""
        export_ok = False
        try:
            export_ok = await run_export(
                url=url,
                browser_path=browser_path,
                target_page=page,
                stop_after_filters=True,
                filter_plan_override=runtime_plan,
        ...
        moved_files = capture_and_move_new_downloads(
        ...
        template_success = bool(export_ok)
        if template_success:
            success += 1
        else:
            failed += 1
        ...
        log.info(
            f"[TEMPLATE_EXEC_RESULT] template_id={template_id} success={template_success} "
            f"export_ok={export_ok} output_folder={output_folder} error={error_msg}"
        )
        if not moved_files:
            log.info(f"[TEMPLATE_EXEC_RESULT] template_id={template_id} moved_files=0")

    log.info(
        f"[BATCH_SUMMARY] total={total} success={success} failed={failed} skipped={skipped}"
    )
...
def main():
    if not validate_runtime_config():
        sys.exit(1)
    try:
        success = asyncio.run(run_export_batch(POWERBI_URL, BROWSER_PATH, TARGET_PAGE))
